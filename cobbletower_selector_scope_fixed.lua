...
local BUILD_RESERVE = 16 -- spare blocks/panes per floor to tolerate terrain/retries
...
local function placeDownSelected()
    local wantedName=selectedItemName()
    if not wantedName then return false end

    if turtle.detectDown() then
        local ok,data=turtle.inspectDown()

        -- Existing correct material is already finished.
        if ok and data and data.name==wantedName then
            return true
        end

        -- Never destroy protected storage.
        if ok and data and isChestName(data.name) then
            error("Refusing to destroy protected chest during construction")
        end
...
local function buildPerimeterLayer(placeFn, selector)
    ...
    local function placeHere()
        if placeFn(px,pz) then
            if not selector() then return false, "Out of selected building material" end
...
local function ensureFloorSupplies(floor)
    ...
    needCobble = needCobble + BUILD_RESERVE
    needPanes = needPanes + BUILD_RESERVE
...
local function buildFloor(floor)
    ...
    print("Supply loaded: cobble="..countCobble().." panes="..countPanes())
