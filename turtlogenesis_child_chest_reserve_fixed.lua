...
function genesis()
    ...
    -- Child bootstrap now reserves two chests: one for its persistent
    -- STORAGE HOME and one for the temporary proxy crafting chest used by
    -- genesis. This prevents the child consuming its only chest for storage.
    local child_chests = itemsBeared(false)['minecraft:chest']
    if not child_chests or child_chests.count < 2 then
        error('Child requires 2 reserved chests before birth; only ' ..
              tostring(child_chests and child_chests.count or 0) .. ' available')
    end
    ...
    if not giveChild('minecraft:chest', 2) then
        error('Reserved child genesis chests are missing at birth')
    end
