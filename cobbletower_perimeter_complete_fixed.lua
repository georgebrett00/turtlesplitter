...
local function buildPerimeterLayer(placeFn, selector)
    -- Explicitly trace all four edges of the 12x12 footprint.
    -- Coordinates are local to the tower:
    -- front-left=(0,0), front-right=(11,0),
    -- back-right=(11,11), back-left=(0,11).
    local px, pz = 0, 0

    local function placeHere()
        if placeFn(px,pz) then
            if not selector() then
                return false, "Out of building material"
            end
            if not placeDownSelected() then
    ...
    -- Place the starting/front-left corner once.
    local ok,err = placeHere()
    if not ok then return false,err end
