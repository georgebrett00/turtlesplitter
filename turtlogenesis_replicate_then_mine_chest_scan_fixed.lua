...
function getFromChest(items_wanted)
    ...
    -- Scan a chest completely rather than repeatedly sucking its first stack.
    -- Unwanted stacks are temporarily kept in the turtle while we continue
    -- searching, so an item deeper in the chest cannot be hidden behind them.
    local function scanChest(direction)
        local scanned = {}

        for attempts = 1, 64 do
        ...
        -- Put only unwanted stacks back after the scan is complete.
        for _, entry in ipairs(scanned) do
            if not entry.keep and turtle.getItemCount(entry.slot) > 0 then
                turtle.select(entry.slot)
                if direction == 'down' then
                    if not turtle.dropDown() then
                        error("Could not return item to lower storage chest")
                    end
                else
                    if not turtle.drop() then
                        error("Could not return item to front proxy chest")
                    end
                end
            end
        end
    ...
    -- The persistent lower chest is searched first, but importantly the scan
    -- continues past its first stack. Then search the front proxy chest too.
    scanChest('down')
    scanChest('front')

    -- Count requested ingredients now present in turtle inventory.
