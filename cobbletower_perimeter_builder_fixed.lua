...
local function scanExistingDepotCobble()
    ...
    withDepotFacing(function()
        -- Temporarily remove chest stacks into separate turtle slots.
        -- Do NOT return them until the scan is finished, otherwise old
        -- ComputerCraft may repeatedly suck the same first stack.
        for tries=1,16 do
            local empty = nil
            for s=1,16 do
                if turtle.getItemCount(s)==0 then
        ...
        -- Restore every stack to the depot after the complete scan.
        for i=1,#held do
...
local function buildPerimeterLayer(placeFn, selector)
    -- Turtle starts at footprint corner (0,0), facing along the front edge.
    local px, pz = 0, 0
    ...
    -- Four sides. Each side moves 11 blocks; corner cells are naturally
    -- shared so the complete 12x12 outer ring contains 44 positions.
    for side=1,4 do
        for step=1,TOWER_SIZE-1 do
            local ok,err=placeHere()
            if not ok then return false,err end

            digForwardMove()

            if facing==0 then pz=pz-1
            elseif facing==1 then px=px+1
            elseif facing==2 then pz=pz+1
            else px=px-1 end
        end
        turnRight()
    end

    -- We have returned to the starting corner; place it once.
    local ok,err=placeHere()
    if not ok then return false,err end

    -- Restore original tower-facing direction.
    face(startFacing)
    return true
end
