...
function returnToSurfaceForGenesis()
    ...
    -- Move horizontally away from the shaft before looking for ground.
    -- Use nodig=true so we do not tunnel through a tree/hill on the surface.
    local moved_forward = 0
    while moved_forward < 3 do
        if go('forward', true) then
            moved_forward = moved_forward + 1
        else
            -- Something blocks this direction. Turn right and try a new
            -- surface direction instead of digging through the obstacle.
            turtle.turnRight()
            logMovement('right')
            print("Surface path blocked; turned right.")
        end
    end
