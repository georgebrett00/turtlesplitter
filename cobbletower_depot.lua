...
local function isTrash(name)
    return name == "minecraft:dirt"
        ...
        or name == "minecraft:cobblestone_wall"
...
local function depositQuarryLoot()
    -- Keep a small fuel reserve in the turtle. Everything else from mining,
    -- including cobble and valuable ores, goes into the permanent depot.
        ...
        for slot=1,16 do
            ...
            if d then
                ...
                if d.name=="minecraft:coal" or d.name=="minecraft:charcoal" then
                    keep = turtle.getFuelLevel() ~= "unlimited" and turtle.getFuelLevel() < 800
                end
                if not keep then
                    turtle.select(slot)
                    turtle.drop()
...
local function suckFromDepot(predicate, wanted)
    ...
    withDepotFacing(function()
        -- Pull stacks one at a time, returning unwanted items immediately.
        for tries=1,64 do
            if got >= wanted then break end
            local empty=nil
            for s=1,16 do if turtle.getItemCount(s)==0 then empty=s break end end
...
local function withdrawCobble(wanted)
    return suckFromDepot(function(n) return n==COBBLE_NAME end, wanted)
end
...
local function depotCountItem(predicate)
    -- Non-invasive exact chest counting is awkward on old ComputerCraft.
    -- Temporarily pull matching stacks, count them, then put them back.
    local total=0
    withDepotFacing(function()
        for tries=1,64 do
            local empty=nil
            ...
            local d=turtle.getItemDetail(empty)
            if d and predicate(d.name) then total=total+d.count end
            turtle.drop()
        end
    end)
    turtle.select(1)
...
local function depotCobbleCount()
    return depotCountItem(function(n) return n==COBBLE_NAME end)
end
