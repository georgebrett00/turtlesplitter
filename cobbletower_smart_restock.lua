...
local function goToDepotFromTower()
    ...
    while true do
        ...
        if not turtle.down() then
            -- Never dig blindly here: the tower may be directly below.
...
local function restockAtDepot(cobbleWanted, panesWanted)
    -- First put leftovers away so all 16 slots are available.
    for slot=1,16 do
        local d=turtle.getItemDetail(slot)
    ...
    if not refuelIfNeeded() then
        ...
        if not refuelIfNeeded() then error("Depot is out of fuel") end
    end

    -- PANES FIRST. They are the scarce/non-mined material and must always
    -- reserve inventory space before cobblestone is loaded.
    local panes=depotBelowTransfer(function(n) return PANE_NAMES[n]==true end,panesWanted)
    if panes < panesWanted then
        error("Depot needs "..(panesWanted-panes).." more glass panes")
    end

    local cobble=depotBelowTransfer(function(n) return n==COBBLE_NAME end,cobbleWanted)
    if cobble < cobbleWanted then
        error("Depot needs "..(cobbleWanted-cobble).." more cobblestone")
    end
end
...
local function returnFromDepotToTower(topY)
    ...
    while y <= topY do up() end
    face(startFacing)
    digForwardMove()
    while y > topY do
        if not turtle.down() then error("Could not return to tower build height") end
        y=y-1
    end
    face(startFacing)
end
