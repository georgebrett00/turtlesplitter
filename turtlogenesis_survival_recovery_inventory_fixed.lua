...
CRAFTING_TREE = {
    ...
    -- Genesis uses two chests (persistent storage + proxy crafting chest),
    -- and two more are reserved for the newborn child's own genesis cycle.
    ['minecraft:chest'] = {count = 4, components = {
        -- Four chests = 32 planks = 8 logs at 4 planks per log.
        -- Reserve the raw wood explicitly so every generation gathers enough
        -- before leaving the surface for genesis.
        ['log'] = {count = 2, subterranean = false},
    }},
...
STATE_FILE = '/.turtlegenesis_state'
MOVE_RETRY_LIMIT = 20
FUEL_SAFETY_MARGIN = 96
PROTECTED_BLOCKS = {
...
function minerRefuel()
    -- Movement refuses to spend the return reserve, so MINER_MIN_FUEL must
    -- never be allowed to sit below MINER_RETURN_FUEL. Refuel toward the
    -- working target instead of stopping as soon as 200 fuel is reached.
    local minimum_safe = math.max(MINER_MIN_FUEL, MINER_RETURN_FUEL + 100)
    if turtle.getFuelLevel() >= minimum_safe then return true end

    for slot = 1, 16 do
        local item = turtle.getItemDetail(slot)
        if item and (item.name == 'minecraft:coal' or item.name == 'minecraft:charcoal') then
            turtle.select(slot)
            while turtle.getItemCount(slot) > 0 and turtle.getFuelLevel() < MINER_MAX_FUEL do
                if not turtle.refuel(1) then break end
            end
            if turtle.getFuelLevel() >= minimum_safe then
                turtle.select(1)
                return true
            end
        end
    ...
    turtle.select(1)
    return turtle.getFuelLevel() >= minimum_safe
end
...
function safeTunnelForward()
    ...
    if turtle.detect() then
        ...
        if ok and data and isProtectedBlockName(data.name) then
            print('Protected block in tunnel: ' .. tostring(data.name))
            return false
        end
        turtle.dig()
        sleep(0.15)
...
function safeVerticalStep(direction)
    ...
    local inspect = direction == 'up' and turtle.inspectUp or turtle.inspectDown
    local dig = direction == 'up' and turtle.digUp or turtle.digDown
    local attack = direction == 'up' and turtle.attackUp or turtle.attackDown
    ...
    -- If there is a block, handle it once. Protected infrastructure is never
    -- destroyed. If there is NO block, the space is probably occupied by a
    -- player/mob/another turtle, so wait instead of declaring the shaft blocked.
    if detect() then
        local ok, data = inspect()
        if ok and data and isProtectedBlockName(data.name) then
            print('Protected block in vertical route: ' .. tostring(data.name))
            return false
        end
        dig()
        sleep(0.15)
    ...
    print('Vertical path temporarily occupied; waiting...')
    for attempt=1,40 do
        sleep(0.5)
        ...
        elseif attempt % 10 == 0 and attack then
            -- Useful for mobs/items. A player merely standing on the turtle
            -- still gets plenty of time to move before this is attempted.
            attack()
        end
    end

    print('Vertical route still occupied after waiting.')
    return false
end
...
function goFromStorageToTrueHome()
    -- Autonomous mining must start from the shaft, never from STORAGE HOME.
    -- Move across the surface without digging so the persistent chest and
    -- surrounding base are never destroyed.
    local guard = 0
    while (home_location.x ~= 0 or home_location.z ~= 0) and guard < 128 do
        guard = guard + 1
        local dx = -home_location.x
        local dz = -home_location.z
        if math.abs(dx) >= math.abs(dz) and dx ~= 0 then
        ...
        if not safeBaseStepForward() then
            print('Surface route from storage to shaft is blocked.')
            return false
        end
    end
    face('north')
...
function fuelNeededToTrueHome(extra)
    local distance = math.abs(home_location.x) +
                     math.abs(home_location.y) +
                     math.abs(home_location.z)
    return distance + (extra or 0) + FUEL_SAFETY_MARGIN
end
...
function autonomousMine()
    ...
    recovery_phase = 'autonomous'
    replication_mode = false
    saveRecoveryState()

    local run = miner_run_number
    while MINER_MAX_RUNS == 0 or run < MINER_MAX_RUNS do
        run = run + 1
        miner_run_number = run
        saveRecoveryState()
        ...
        -- After the previous run we are parked over STORAGE HOME. Move back
        -- to the TRUE HOME shaft before descending. Previously `location` was
        -- reset while the turtle was still 3 blocks away over the chest,
        -- causing it to dig straight down through the persistent chest.
        if home_location.x ~= 0 or home_location.z ~= 0 then
            print('Leaving storage; returning to mining shaft')
            if not goFromStorageToTrueHome() then
                print('Could not reach mining shaft - stopping')
                break
            end
        end

        -- We are now physically at TRUE HOME, so it is safe to re-anchor the
        -- mining coordinate system and descend through the shaft.
        location = {x = 0, y = 0, z = 0}

        -- Always leave the surface through the same vertical shaft and
        -- perform the actual mining at the configured relative mining depth.
        ...
        if branch_offset > 0 then
            ...
            while location.x < base.x + branch_offset do
                if turtle.getFuelLevel() < math.max(MINER_RETURN_FUEL, fuelNeededToTrueHome(16)) then
                    branch_ok = false
                    break
                end
        ...
        if not completed then
            print('Branch ended early; returning safely and continuing next run.')
        end

        -- Return from the branch by RETRACING the known-open tunnel.
        -- The old coordinate mover could choose a direct route through fresh
        -- rock/protected terrain and fail even though the corridor we just
        -- mined was perfectly usable.
        face('south')
        local branch_return_ok = true
        while location.z ~= branch_start.z do
            if turtle.getFuelLevel() < MINER_RETURN_FUEL then
                branch_return_ok = false
                break
            end
            if not safeTunnelForward() then
                branch_return_ok = false
        ...
        if not branch_return_ok then
            print('Could not retrace mining branch - stopping')
            break
        end

        -- Retrace the east/west branch-offset corridor back to the shaft.
        if location.x ~= base.x then
            face(location.x > base.x and 'west' or 'east')
            local shaft_return_ok = true
            while location.x ~= base.x do
                if turtle.getFuelLevel() < MINER_RETURN_FUEL then
                    shaft_return_ok = false
                    break
                end
                if not safeTunnelForward() then
                    shaft_return_ok = false
            ...
            if not shaft_return_ok then
                print('Could not retrace branch corridor to shaft - stopping')
                break
            end
        end

        -- We are now physically at the shaft at mining depth. Climb the
        -- already-dug shaft rather than asking generic coordinate navigation
        -- to rediscover it.
...
-- Persistent recovery metadata. Movement checkpoints keep the stored position
-- synchronized closely enough to return HOME after a reboot/server restart.
recovery_phase = 'replicating'
miner_run_number = 0

function isProtectedBlockName(name)
    return name and PROTECTED_BLOCKS[name] == true
end

function saveRecoveryState()
    if not fs or not textutils then return end
    local state = {
        location = location,
        ...
        miner_run_number = miner_run_number,
    }
...
function loadRecoveryState()
    ...
    if type(state.location) == 'table' then location = state.location end
    if type(state.home_location) == 'table' then home_location = state.home_location end
    if type(state.storage_home) == 'table' then storage_home = state.storage_home end
    if state.orientation then orientation = state.orientation end
    calibrated = state.calibrated == true
    replication_mode = state.replication_mode ~= false
    recovery_phase = state.recovery_phase or 'replicating'
    miner_run_number = tonumber(state.miner_run_number) or 0

    print('Recovery state loaded: ' .. tostring(recovery_phase))
    return true
end
...
function go(direction, nodig)
    if not nodig then
        if DETECT[direction] and DETECT[direction]() then
            ...
            if inspect then ok, data = inspect() end
            if ok and data then
                local protected = isProtectedBlockName(data.name)

                -- Chests need special treatment during genesis. The permanent
                -- STORAGE HOME chest is sacred, but temporary/proxy crafting
                -- chests are deliberately dismantled by the original genesis
                -- choreography.
                if recovery_phase == 'genesis' and
                   (data.name == 'minecraft:furnace' or
                    data.name == 'computercraft:peripheral' or
                    data.name == 'computercraft:disk_drive') then
                    protected = false
                end

                if data.name == 'minecraft:chest' or
                   data.name == 'minecraft:trapped_chest' then
                    protected = false
                    if storage_home then
                        local target = getAdjacentBlock(direction)
                        if target and
                           target.x == storage_home.x and
                           target.z == storage_home.z and
                           target.y == storage_home.y - 1 then
                            protected = true
                        end
                    end
                end
                ...
                if protected then
                    print('Protected block ahead: ' .. tostring(data.name))
                    return false
                end
            end
            if DIG[direction] then DIG[direction]() end
        end
    end
...
function goAbsolute(direction)
    for attempt=1,MOVE_RETRY_LIMIT do
        if go(direction) then return true end
        sleep(0.25)
    end
    error('Movement failed after retries: ' .. tostring(direction) ..
          ' - stopping before position becomes unsafe')
end
...
function trimReplicationCobblestone()
    if not replication_mode then return end

    local kept = 0
    for slot=1,16 do
        local d=turtle.getItemDetail(slot)
        if d and interpretItemName(d.name) == 'minecraft:cobblestone' then
            if kept < 64 then
                local room = 64-kept
                if d.count <= room then
                    kept = kept+d.count
                else
                    turtle.select(slot)
                    -- Keep only the amount needed to bring the retained total
                    -- to one stack; discard the surplus from this slot.
                    if room > 0 then
                        -- Find/merge into an earlier retained cobble stack.
                        for target=1,slot-1 do
                            local td=turtle.getItemDetail(target)
                            if td and interpretItemName(td.name)=='minecraft:cobblestone' then
                                turtle.transferTo(target, room)
                                break
                            end
                        end
                    end
...
function cleanReplicationInventory()
    ...
    -- Surface resources such as logs/reeds/sand take priority over surplus
    -- cobblestone. Keep at most one cobble stack while gathering.
    trimReplicationCobblestone()
...
function genesis()
    ...
    -- Clear a real block once, but an entity (cow/player/etc.) is invisible
    -- to detect(). In that case wait for it to move instead of aborting genesis.
    if turtle.detect() then
        turtle.dig()
        sleep(0.15)
    end

    local proxy_placed = false
    local proxy_wait_message = false
    for attempt = 1, 60 do
        ...
        if not turtle.detect() then
            if not proxy_wait_message then
                print("Proxy chest position occupied; waiting...")
                proxy_wait_message = true
            end
            -- Try to clear a mob. This also permits a player simply to step
            -- aside; failure to attack is harmless.
            if attempt % 10 == 0 then
                turtle.attack()
            end
        else
            -- A block has appeared in the target position. Try once to clear
            -- ordinary terrain; protected blocks will simply remain.
            turtle.dig()
        end
        sleep(0.5)
    end

    if not proxy_placed then
        error("Proxy chest position remained blocked for 30 seconds")
    end
    print("Proxy crafting chest placed.")
    ...
    local function giveChildAcrossStacks(item_name, amount)
        local remaining = amount
        for slot = 1, 16 do
            if remaining <= 0 then break end
            local d = turtle.getItemDetail(slot)
            if d and interpretItemName(d.name) == interpretItemName(item_name) then
                turtle.select(slot)
                local before = turtle.getItemCount(slot)
                local send = math.min(before, remaining)
                if send > 0 and turtle.dropDown(send) then
        ...
        turtle.select(1)
        if remaining > 0 then
            error('Reserved child ' .. tostring(item_name) ..
                  ' short by ' .. tostring(remaining))
        end
        print('Gave child: ' .. tostring(item_name) .. ' x' .. tostring(amount))
        return true
    end

    -- The child needs two physical chests. They may be split across multiple
    -- inventory slots, so transfer them cumulatively rather than trusting the
    -- single-stack summary returned by itemsBeared().
    ...
    giveChildAcrossStacks('minecraft:chest', 2)
...
function countInventoryItem(item_name)
    local total = 0
    local wanted = interpretItemName(item_name)
    for slot=1,16 do
        local d = turtle.getItemDetail(slot)
        if d and interpretItemName(d.name) == wanted then
            total = total + d.count
        end
    end
    return total
...
function ensureGenesisChestReserve()
    -- At STORAGE HOME the permanent chest has already been placed/consumed.
    -- We still require:
    --   1 proxy crafting chest for THIS genesis
    --   2 chests to hand to the newborn
    -- Never begin genesis with fewer than 3 chests physically in inventory.
    -- The resource tree now gathers enough wood for all four chests up front;
    -- this remains only as a final safety check for unexpected item loss.
    local have = countInventoryItem('minecraft:chest')
    if have >= 3 then return true end

    print('Need ' .. tostring(3-have) ..
          ' more chest(s) before genesis; resuming resource gathering.')
    return false
end
    ...
    while true do
        ...
        if not refueled or need_subterranian_items then
            ...
            if need_surface_items then
                ...
                -- Surface resources such as logs, reeds and sand must never be
                -- searched for from the underground mining tunnel.  The old
                -- lifecycle called explore() immediately, even when mine()
                -- had left the turtle deep underground.
                print("Surface resources required; climbing to surface first...")
                returnToSurfaceForGenesis()
                ...
                -- Placing STORAGE HOME consumes one of the four gathered
                -- chests. Verify the three remaining physical chests before
                -- genesis: one proxy + two reserved for the newborn.
                if not ensureGenesisChestReserve() then
                    -- Return to the normal lifecycle. The resource tree still
                    -- knows chests are required, and gathering will continue
                    -- rather than entering genesis under-supplied.
                    storage_home = storage_home
                else
                    print("Beginning genesis at STORAGE HOME...")
                    recovery_phase = 'genesis'
                    saveRecoveryState()
                    genesis()
                    replication_mode = false
                    recovery_phase = 'autonomous'
                ...
                -- If genesis did not run (for example because the final chest
                -- reserve check failed), continue the gathering lifecycle.
                if replication_mode then
                    print('Genesis not yet complete; continuing resource gathering.')
                else
                    print('Replication complete.')
                    print('Returning to this turtle\'s spawn/base, then mining.')
                    autonomousMine()
...
function initiate()
    ...
    -- Do not trim a fixed number of characters from the end of this program.
    -- Older builds ended in userInit(), so sub(1,-11) happened to work.
    -- The recovery build has a larger startup block; fixed-length trimming
    -- chopped off its final `end` and produced the EOF syntax error.
    -- Use a single-line anchor for compatibility with older CraftOS/Lua.
    -- Searching for a multiline marker proved unreliable on this installation.
    local boot_marker = "nameTurtle()"
    local marker_start = nil
    local search_from = 1
    while true do
        local found = string.find(file_contents, boot_marker, search_from, true)
        if not found then break end
        marker_start = found
        search_from = found + 1
    end
    if not marker_start then
        error('Unable to locate nameTurtle startup marker')
    end

    file_contents = string.sub(file_contents, 1, marker_start - 1)
    file_contents = file_contents .. [[nameTurtle()
local recovered = loadRecoveryState()
if recovered and recovery_phase == 'autonomous' and not replication_mode then
    print('Resuming autonomous worker after restart...')
    autonomousMine()
else
    main()
end
