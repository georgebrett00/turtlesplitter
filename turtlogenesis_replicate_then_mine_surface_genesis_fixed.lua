...
function returnToSurfaceForGenesis()
    ...
    -- If we calibrated underground, location.y is now meaningful relative to
    -- the bedrock calibration. Move upward until the turtle reaches open air.
    -- We deliberately do NOT use the old ore-depth table here.
    local climbed = 0
    local open_air_steps = 0
    local max_climb = 384

    while climbed < max_climb do
        local blocked_up = turtle.detectUp()

        if blocked_up then
            -- Dig the vertical escape shaft.
            if not go('up') then
                sleep(0.2)
            else
                climbed = climbed + 1
            ...
            -- Move through open space. Requiring several consecutive clear
            -- upward moves avoids mistaking a small underground cave for the
            -- surface.
            if go('up') then
                climbed = climbed + 1
                open_air_steps = open_air_steps + 1
            else
                sleep(0.2)
            end

            if open_air_steps >= 4 then
                print("Surface/open air reached.")
                print("Climbed " .. tostring(climbed) .. " blocks.")
                return true
            end
        end
    end

    error("Could not reach surface safely before genesis")
end
