...
function minerStripBranch()
    ...
    for i = 1, MINER_TUNNEL_LENGTH do
        ...
        if not turtle.forward() then
            -- Try the existing movement helper once the obstruction is removed.
...
function autonomousMine()
    ...
    if not minerReturnToBase(base) then
    ...
    while MINER_MAX_RUNS == 0 or run < MINER_MAX_RUNS do
        ...
        if branch_offset > 0 then
            ...
            if not minerMoveToCoordinate({x = base.x + branch_offset, y = MINER_DEPTH, z = base.z}) then
                print('Could not reach branch start - stopping')
                break
            end
            if not face('north') then break end
        end
        ...
        -- Return to the surface/base using the same shaft.
        if not minerReturnToBase(base) then
        ...
        -- Deposit everything into the chest beneath the turtle.
        itemsBeared(true)
...
function getFromChest(items_wanted)
    ...
    -- Find a genuinely empty turtle inventory slot. This is local because the
    -- original turtlogenesis program has no global firstEmptySlot() helper.
    ...
    -- Pull from BOTH genesis storage locations. The lower chest contains
    -- resources carried into genesis, while the front proxy chest contains
    -- items temporarily deposited during crafting.
    local function collectFrom(direction)
        for attempts = 1, 128 do
            ...
            if detail then
                ...
                if not items_wanted[interpreted] and not items_wanted[detail.name] then
    ...
    -- Important: do not stop searching just because one chest supplied an
    -- item. A single recipe may need ingredients split between both chests.
    collectFrom('down')
    collectFrom('front')

    local items = itemsBeared(false)

    -- Return unwanted items safely. Prefer the chest they can always reach.
    for _, unwanted_slot in pairs(unwanted) do
        local detail = turtle.getItemDetail(unwanted_slot)
        if detail then
            ...
            if front_ok and front_data and front_data.name == 'minecraft:chest' then
                ...
                if down_ok and down_data and down_data.name == 'minecraft:chest' then
                    ...
                    error("No chest available to return unwanted crafting item")
                end
            end
        end
    end

    -- Verify every requested ingredient against the combined inventory.
    for wanted_name, wanted_count in pairs(items_wanted) do
        ...
        for slot = 1, 16 do
            ...
            if detail then
                local interpreted = interpretItemName(detail.name)
                if interpreted == wanted_name or detail.name == wanted_name then
...
function mine()
    ...
    while next(lacked) ~= nil or not refuelGood do
        ...
        if i >= HOUSEKEEP_FREQUENCY then
            ...
            -- Deposit/merge what was collected, then recalculate requirements.
...
function dumpExcept(exceptions)
    local front_ok, front_data = turtle.inspect()
    local proxy_in_front = front_ok and front_data and front_data.name == 'minecraft:chest'
            ...
            if not exceptions[name] then
                ...
                if proxy_in_front then
                    ...
                    if down_ok and down_data and down_data.name == 'minecraft:chest' then
                        ...
                        error("Refusing to dump crafting items: no chest available")
                    end
                end
            end
        end
...
function returnToSurfaceForGenesis()
    ...
    -- First escape the underground. Several consecutive clear upward moves
    -- distinguish open sky from a small cave.
    while climbed < max_climb and clear_up < 4 do
    ...
    if clear_up < 4 then
        error("Could not reach open air safely before genesis")
    end

    print("Open air reached.")
    print("Clearing escape shaft rim...")

    -- We may only have reached a narrow vertical opening. Move UP a little
    -- farther before attempting horizontal travel so the turtle is actually
    -- above the surrounding terrain rather than still inside the shaft.
    local extra_up = 0
    while extra_up < 3 do
        if go('up') then
            extra_up = extra_up + 1
    ...
    -- Try each horizontal direction at most once. This avoids the previous
    -- infinite turn-right loop when all four sides are blocked.
    local moved_forward = 0
    local turns_without_move = 0

    while moved_forward < 3 do
        if go('forward', true) then
            moved_forward = moved_forward + 1
            turns_without_move = 0
        else
            turtle.turnRight()
            logMovement('right')
            turns_without_move = turns_without_move + 1

            if turns_without_move >= 4 then
                -- All four horizontal sides are blocked even after rising
                -- above the shaft. Rise another block and retry.
                print("All sides blocked; rising above terrain...")
                if go('up') then
                    turns_without_move = 0
                else
                    error("Unable to escape surface shaft")
                end
            end
        end
    end

    print("Moved " .. tostring(moved_forward) .. " blocks away from shaft.")
    print("Searching for dry land...")

    -- Do not start genesis over water. Water does not count as a safe
    -- foundation for the storage chest, and dropping items here can lose them.
    -- Travel forward until the column below the turtle leads to solid ground
    -- without the turtle itself entering water.
    local dry_search = 0
    ...
    while dry_search < max_dry_search do
        ...
        local water_below = down_name == 'minecraft:water' or
                            down_name == 'minecraft:flowing_water'

        if not water_below then
            break
        end

        -- Stay above the water surface and move horizontally. Do not dig.
        if go('forward', true) then
            dry_search = dry_search + 1
        else
            -- Try another direction rather than tunnelling through terrain.
            turtle.turnRight()
            logMovement('right')
        end
    end
    ...
    print("Dry landing column found.")
    print("Descending to ground level...")

    -- Descend until there is a solid block directly underneath.
    -- This prevents genesis/crafting from happening while floating in the air.
    local descended = 0
    while not turtle.detectDown() do
        ...
        if descended > max_climb then
            error("Could not find ground below surface position")
        end
    end

    print("Ground reached.")
    print("Turtle is one block above solid ground.")
    return true
end
...
function ensureChestBelow()
    ...
    if ok and data and (
        ...
        error("Unsafe genesis location: liquid directly below turtle")
    end

    -- Never drop inventory into open air or onto an arbitrary block.
    -- If there is ground underneath, make a one-block storage recess and
    -- place a chest into it so the turtle remains directly above the chest.
    if ok then
        if not turtle.digDown() then
            error("Cannot prepare storage position below turtle")
        end
    end

    if not selectItem('minecraft:chest') then
        error("No chest available to place below turtle")
    end

    if not turtle.placeDown() then
        error("Failed to place storage chest below turtle")
    end

    local chest_ok, chest_data = turtle.inspectDown()
    if not (chest_ok and chest_data and chest_data.name == 'minecraft:chest') then
        error("Storage chest verification failed")
    end

    print("Storage chest placed and verified.")
    return true
end
