...
function minerReturnToBase(base)
    -- Use the existing route system to return to the recorded base position.
    local route = fastestRoute(
        {[strXYZ(location)] = true},
        {[strXYZ(base)] = true},
        location,
        orientation
    )
    if route then
        followRoute(route)
        return strXYZ(location) == strXYZ(base)
    end
    return false
end
...
function autonomousMine()
    ...
    -- The point at which the child enters worker mode becomes its home/base.
    local base = {x = location.x, y = location.y, z = location.z}
    local base_orientation = orientation
    ...
    while MINER_MAX_RUNS == 0 or run < MINER_MAX_RUNS do
        ...
        local start = {x = location.x, y = location.y, z = location.z}
        local start_orientation = orientation

        print('Mining run ' .. tostring(run))

        -- A simple branch miner is deliberately used here rather than
        -- replacing the proven replication/mining machinery above.
        local completed = minerStripBranch()

        -- Return to the beginning of this branch.
        local route = fastestRoute(
            {[strXYZ(location)] = true},
            {[strXYZ(start)] = true},
            location,
        ...
        -- Then return to the worker's permanent base.
        if not minerReturnToBase(base) then
            print('Could not route to base - stopping')
            break
        end
...
function genesis()
    ...
    -- Mark the new turtle as an autonomous mining worker.
    -- Its startup program will consume this marker on first boot.
    local worker_file = fs.open('/disk/autonomous_miner', 'w')
    worker_file.write('1')
    worker_file.close()
...
function main()
    -- A replicated turtle is marked as a worker by the replication process.
    -- Worker turtles skip the replication loop and become autonomous miners.
    if fs.exists('/autonomous_miner') and AUTONOMOUS_MINING then
        fs.delete('/autonomous_miner')
        autonomousMine()
        return
    end
...
function populateDisk()
    ...
    -- Child turtles use this marker to enter autonomous mining mode.
    file = fs.open('/disk/autonomous_miner', 'w')
    file.write('1')
    file.close()
end
