...
function getFromChest(items_wanted)
    ...
    local function pullFrom(direction)
    ...
    -- Persistent resources always live in the chest below.
    pullFrom('down')

    -- The front chest is only a crafting/proxy workspace. We still search it
    -- because an intermediate crafted component may have been parked there.
    pullFrom('front')

    -- Return anything not requested to persistent storage BELOW, never to the
    -- proxy chest. This consolidates resources instead of splitting them.
    for _, slot in pairs(unwanted) do
        if turtle.getItemCount(slot) > 0 then
            turtle.select(slot)
            ...
            if down_ok and down_data and down_data.name == 'minecraft:chest' then
                ...
                error("Persistent storage chest missing below turtle")
            end
        end
    end

    -- Verify requested materials across the turtle inventory.
