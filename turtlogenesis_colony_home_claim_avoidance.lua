...
function returnHomeSafely()
    print('Returning to HOME safely...')
    -- Use a consistent underground transport layer.
    while location.y > HOME_TRAVEL_DEPTH do
        if not safeVerticalStep('down') then break end
    end
    while location.y < HOME_TRAVEL_DEPTH do
        if not safeVerticalStep('up') then break end
    end
    if not navigateHorizontalWithDetours({x=0,z=0}) then return false end
    while location.y < 0 do
        if not safeVerticalStep('up') then print('HOME shaft blocked.'); return false end
    end
    while location.y > 0 do
        if not safeVerticalStep('down') then print('HOME shaft blocked.'); return false end
    end
    face('north')
    print('HOME reached.')
    ...
    while true do
        ...
        if not refueled or need_subterranian_items then
            ...
            if need_surface_items then
                ...
                ensureChestBelow()
                print("Beginning genesis at HOME...")
                genesis()
