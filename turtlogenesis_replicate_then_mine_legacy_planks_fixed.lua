...
function getFromChest(wanted_items)
    ...
    while slot <= 16 and remaining_count > 0 do
        if turtle.getItemCount(slot) == 0 then
            ...
            -- Genesis can use two chests:
            --   * persistent resource storage directly below
            --   * the temporary/proxy crafting chest directly in front
            -- Resources collected before genesis are normally in the lower
            -- chest, while dumpExcept() uses the forward proxy chest.
